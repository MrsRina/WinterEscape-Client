package me.rina.hyperpop.impl.gui.api.imperador.widget;
...
public class ImperadorEntryBox extends Widget {
    ...
    public void split(float k) {
        ...
        if (this.isSplitRendering()) {
            Processor.prepare(this.string[0], this.string[1], this.string[2], (int) this.string[3]);
            Processor.solid(this.rect.getX() + this.offsetX + k, this.rect.getY() + 1f, 1f, this.rect.getHeight() - 2f);
        }
    }
                ...
                switch (key) {
                    ...
                    default: {
                        if (Keyboard.isKeyDown(Keyboard.KEY_LCONTROL) || Keyboard.isKeyDown(Keyboard.KEY_RCONTROL)) {
                            ...
                            final String copy = ImperadorUtil.get();
                            ...
                            if (key == Keyboard.KEY_C && this.getIndexA() != this.getIndexB()) {
                                ImperadorUtil.set(cache.substring(this.indexA, this.indexB));
                            }
    ...
    public void onRender() {
        ...
        Processor.prepare(this.background[0], this.background[1], this.background[2], (int) this.background[3]);
        Processor.solid(this.rect);

        if (this.lastTickOutlineAlpha != 0f && this.isToDrawOutline()) {
            Processor.prepare(this.outline[0], this.outline[1], this.outline[2], (int) this.lastTickOutlineAlpha);
            Processor.outline(this.rect);
        }

        if (this.lastTickFocusedAlpha != 0f) {
            Processor.prepare(this.focused[0], this.focused[1], this.focused[2], (int) this.lastTickFocusedAlpha);
            Processor.solid(this.rect);
        }

        if (this.lastTickPressedAlpha != 0f) {
            Processor.prepare(this.pressed[0], this.pressed[1], this.pressed[2], (int) this.lastTickPressedAlpha);
            Processor.solid(this.rect);
        }
        ...
        for (String c : this.text.split("")) {
            ...
            if (j > this.getIndexA() && j <= this.getIndexB() && this.getIndexB() != this.getIndexA() && this.isFocused()) {
                Processor.prepare(this.background_selected[0], this.background_selected[1], this.background_selected[2], (int) this.background_selected[3]);
                Processor.solid(this.rect.getX() + this.offsetX + k, this.rect.getY() + this.offsetY / 2, w + this.getKerning(), this.rect.getHeight() - (this.offsetY / 2 * 2));

                l = k + w + this.getKerning();
                r = false;
            }

            Processor.string(this.getFont(), c, this.rect.getX() + this.offsetX + k, this.rect.getY() + this.offsetY, TurokGL.arrayColorToColorClass(this.background));
